...
void setup() 
{
    int CalcMinMax(int leiturasdia[], int *tempmin, int *tempmax); // protótipo da função
    int lowtemp;
    int hitemp;
    int valorretido; // serve apenas para charmar a função e recebe sempre o valor 24
    ...
    valorretido = CalcMinMax(leiturasdia, &lowtemp, &hitemp);  /* Referenciar arrays sem [], no caso, o array temperaturas[], significa referenciar o seu endereço inicial.
    ...
    Serial.print((long)lowtemp, DEC);
    Serial.print("\n");
    Serial.print("Variável Valorretido eh: ");
    Serial.print((int)valorretido, DEC);
...
int CalcMinMax(int leiturasdia[], int *tempmin, int *tempmax) //calcula as temperaturas mínima e máxima, lendo TEMPERATURAS POR DIA
{
  int j;
  *tempmin = BAIXATEMPERATURA; // ver #define: recebe o valor de 63
  *tempmax = ALTATEMPERATURA;  // ver #define: recebe o valor de 76
  for (j=0; j<LEITURASPORDIA;j++)
  {
  ...
  return j; // retorna sempre o valor máxima do contador, dado por LEITURAS POR DIA, ou seja, 24
 }
